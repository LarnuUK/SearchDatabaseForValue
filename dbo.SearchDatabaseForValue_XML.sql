CREATE OR ALTER PROC [dbo].[SearchDatabaseForValue_XML] @DatabaseName sysname, --Name of database to be searched
...
BEGIN
/*
Written by Thom A 2025-05-13
Licenced under CC BY-ND 4.0
Targets SQL Server 2008+
*/
    SET NOCOUNT ON;

    /*
    DECLARE @DatabaseName sysname = N'Sandbox',
            @SearchValue sql_variant = 'abc',
    ...
    SET @OuterSQL = N'DECLARE @WhereDelimiter nvarchar(30) = @CRLF + N''   OR '';' + @CRLF +
                    ...
                    N'           N''WHERE '' + STUFF((SELECT @WhereDelimiter + ' + @CRLF +
                    ...
                    N'                                FROM sys.columns c' + @CRLF +
                    N'                                     JOIN sys.types ct ON c.user_type_id = ct.user_type_id' + @CRLF +
                    N'                                     JOIN sys.types st ON ct.system_type_id = st.user_type_id' + @CRLF +
                    N'                                WHERE c.object_id = t.object_id' + @CRLF +
                    N'                                  AND (st.name IN (CONVERT(sysname,SQL_VARIANT_PROPERTY(@SearchValue,''BaseType'')),REPLACE(CONVERT(sysname,SQL_VARIANT_PROPERTY(@SearchValue,''BaseType'')),''var'',''''))' + @CRLF +
                    ...
                    N'                                FOR XML PATH(''''),TYPE).value(''(./text())[1]'',''nvarchar(MAX)''),1,DATALENGTH(@WhereDelimiter)/2,N'''') + N'';'' + @CRLF +' + @CRLF +
                    ...
                    N'             t.object_id,' + @CRLF +
                    N'             t.name)' + @CRLF +
                    N'SELECT @InnerSQL = STUFF((SELECT @CRLF + S.Statement' + @CRLF +
                    N'                          FROM Statements S' + @CRLF + 
                    N'                          FOR XML PATH(''''),TYPE).value(''(./text())[1]'',''nvarchar(MAX)''),1,2,N'''')' + @CRLF +
                    N'IF @WhatIf = 0' + @CRLF + 
